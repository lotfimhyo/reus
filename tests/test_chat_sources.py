...
def test_chat_response_preserves_actual_sources_only():
    response = ChatResponse.from_executor_result({"response": "answer", "sources": ["memory:evidence-7", "https://example.org/source", {"ignored": True}]})
...
def test_chat_response_does_not_invent_sources_for_plain_text():
    assert ChatResponse.from_executor_result("simple answer").sources == []
